(define (domain Waste_domain)
  ...
  (:predicates
    ...
    (old_bag_dumb ?bin) ; Old Bag Dumb (Old Bag Dumb into Citybins)
    ...
    (between ?truck - Truck ?q_less_one ?q - quantity) ;; It is controlled the capability when the truck will come to deposite the citybins garbage to the dumpyard
  ...
  (:action Move_Person_To_CityBin
      ...
      :precondition (and(related ?who ?person_capacity)(related ?who ?bin)(related ?bin ?OldBag)(person_hands_full ?person_capacity)(related ?cityBin ?bin)(have_oldBag ?OldBag)(is_loc ?who ?from)(is_loc ?cityBin ?to))
  ...
  (:action Load_City_Garbage
    :parameters (?truck - Truck ?q ?q_less_one - quantity ?bin - DustBin ?cityBin - CityBin ?from ?to - location)
    :precondition (and(plus1 ?q_less_one ?q)(Truck_capacity ?truck ?q)(old_bag_dumb ?bin)(related ?cityBin ?bin)(related ?truck ?cityBin)(is_loc ?truck ?from)(is_loc ?cityBin ?to))
    :effect (and(not(is_loc ?truck ?from))(is_loc ?truck ?to)(not(old_bag_dumb ?bin))(collected_cityBins_garbage ?truck)
